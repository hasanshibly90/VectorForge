...
============================================
Production pipeline using potrace for mathematically optimal Bézier curves.
...
def load_and_upscale(input_path: str, target: str = "4K") -> Image.Image:
    ...
    print(f"  Upscaling: {w0}x{h0} → {new_w}x{new_h} (×{scale:.2f})")
...
def smooth_mask_edges(mask: np.ndarray, sigma: float = 2.0) -> np.ndarray:
    ...
    can fit clean Bézier curves through. The 0.5 threshold keeps the
...
def potrace_to_svg(
    ...
    Potrace fits mathematically optimal cubic Bézier curves to the boundary.
...
def run_cnc_pipeline(
    ...
    print(f"\n[6/7] Smoothing edges (σ={gaussian_sigma}) & tracing with potrace...")
    layer_svgs = {}
    layer_info = []
    ...
    print(f"  BMP: extra smoothing (σ={gaussian_sigma_bmp}) + speck cleanup...")
    ...
    if border_stray > 0:
        print(f"  ⚠ Border has {border_stray} non-white pixels (may be expected if design reaches edge)")
    else:
        print(f"  ✓ Border clean: 0 stray pixels")
    ...
    print(f"\n{'='*60}")
    print(f"  ✅ CNC-GRADE PIPELINE COMPLETE")
